...
def plot_mask_on_image(img_data, mask_data, cm_coords, alpha=0.5, title="", axes_to_display=['sagittal', 'coronal', 'axial'], interpolation='none'):
    """
    ...
    plane_funcs = {
        'sagittal': lambda a: (a.imshow(img_data[x, :, :], cmap='gray'),
                                a.imshow(mask_data[x, :, :], alpha=alpha, cmap='Reds', interpolation=interpolation),
                                a.set_title(f"Sagittal (x={x})")),
        'coronal': lambda a: (a.imshow(img_data[:, y, :], cmap='gray'),
                                a.imshow(mask_data[:, y, :], alpha=alpha, cmap='Reds', interpolation=interpolation),
                                a.set_title(f"Coronal (y={y})")),
        'axial': lambda a: (a.imshow(img_data[:, :, z], cmap='gray'),
                            a.imshow(mask_data[:, :, z], alpha=alpha, cmap='Reds', interpolation=interpolation),
