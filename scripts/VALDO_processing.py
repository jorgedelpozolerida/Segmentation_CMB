...
To implement:
Move studies to new  restructured directory 
Clean data
...
# TODO:
- Implement preprocessing of images:
1. Exclude subjects without labels?
2. Clean MRI images with nan values -> either interpolate missing values or 
exclude subjects
3. Reshape all MRI images to same size (upsample probably)
4. Transform all MRI images to isotropic space
5. Clean label maps: not consistent label value for the CMB
6. Stack together T2s, T2, T1 into single nifti
...
def process_study(args, subject):
    ...
    new_subject_dir = os.path.join(args.out_dir, subject)
    
    if args.process_niftis:
        ensure_directory_exists(os.path.join(new_subject_dir, "label"))
        ensure_directory_exists(os.path.join(new_subject_dir, "mri"))
    ...
    # Additional nifti processing can be added here
    if args.process_niftis:
        pass
...
def main(args):
    ...
    df_global.to_csv(f'../data/{args.dataset_name}_metadata.csv', index=False)
...
def parse_args():
    ...
    parser.add_argument('--process_niftis', action='store_true', default=False,
                        help='Add this flag if you want to also process and move nifties')
